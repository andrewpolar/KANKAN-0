...
public:
	...
	void Input2Output(const std::unique_ptr<double[]>& input, std::unique_ptr<double[]>& output, 
		std::unique_ptr<std::unique_ptr<int[]>[]>& indexes, std::unique_ptr<std::unique_ptr<double[]>[]>& offsets) {
		for (int i = 0; i < _urysohns.size(); ++i) {
			output[i] = _urysohns[i]->GetUrysohn(input, indexes[i], offsets[i]);
		}
	}
	...
	void Update(const std::unique_ptr<double[]>& input, const std::unique_ptr<double[]>& deltas, double mu, 
		const std::unique_ptr<std::unique_ptr<int[]>[]>& indexes, const std::unique_ptr<std::unique_ptr<double[]>[]>& offsets) {
		for (int i = 0; i < _urysohns.size(); ++i) {
			_urysohns[i]->Update(deltas[i] * mu, input, indexes[i], offsets[i]);
		}
	}
	void IncrementPoins() {
		for (int i = 0; i < _urysohns.size(); ++i) {
