...
public:
	...
	double GetUrysohn(const std::unique_ptr<double[]>& inputs, std::unique_ptr<int[]>& indexes, std::unique_ptr<double[]>& offsets) {
		double f = 0.0;
		for (int i = 0; i < _functionList.size(); ++i) {
			f += _functionList[i]->GetFunction(inputs[i], indexes[i], offsets[i]);
		}
		return f;
	}
	...
	void Update(double delta, const std::unique_ptr<double[]>& inputs, const std::unique_ptr<int[]>& indexes,
		const std::unique_ptr<double[]>& offsets) {
		for (int i = 0; i < _functionList.size(); ++i) {
			_functionList[i]->Update(inputs[i], delta, indexes[i], offsets[i]);
		}
	}
