...
void TestSingleFunction() {
	...
	for (int epoch = 0; epoch < 20; ++epoch) {
		...
		for (int i = 0; i < nTrainingRecords; ++i) {
			int index;
			double offset;
			double model = f->GetFunction(features_training[i], index, offset);
			double residual = targets_training[i] - model;
			f->Update(features_training[i], residual * mu, index, offset);
...
void TestSingleUrysohn() {
	...
	double mu = 0.01;
	auto offsets = std::make_unique<double[]>(nFeatures);
	auto indexes = std::make_unique<int[]>(nFeatures);
	for (int epoch = 0; epoch < 10; ++epoch) {
		double error = 0.0;
		for (int i = 0; i < nTrainingRecords; ++i) {
			double model = urysohn->GetUrysohn(features_training[i], indexes, offsets);
			double residual = targets_training[i] - model;
			error += residual * residual;
			urysohn->Update(residual * mu, features_training[i], indexes, offsets);
		}
		error /= nTrainingRecords;
		error = sqrt(error);
		error /= (targetMax - targetMin);
...
void TestLayer() {
	...
	auto offsets = std::make_unique<std::unique_ptr<double[]>[]>(nTargets);
	for (int i = 0; i < nTargets; ++i) {
		indexes[i] = std::make_unique<int[]>(nFeatures);
		offsets[i] = std::make_unique<double[]>(nFeatures);
	}
	...
	for (int epoch = 0; epoch < 10; ++epoch) {
		double error = 0.0;
		for (int i = 0; i < nTrainingRecords; ++i) {
			layer0->Input2Output(features_training[i], models0, indexes, offsets);
			for (int j = 0; j < nTargets; ++j) {
				deltas0[j] = targets_training[i][j] - models0[j];
				error += deltas0[j] * deltas0[j];
			}
			layer0->Update(features_training[i], deltas0, mu, indexes, offsets);
		}
		error /= nTargets;
		error /= nTrainingRecords;
		error = sqrt(error);
...
void Det_4_4() {
	...
	for (int epoch = 0; epoch < 50; ++epoch) {
		for (int i = 0; i < nTrainingRecords; ++i) {
			...
			layer1->Update(models0, deltas1, 0.005, indexes1, offsets1);
			layer0->Update(features_training[i], deltas0, 1.0, indexes0, offsets0);
		}
